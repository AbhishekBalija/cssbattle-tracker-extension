 ...
 * Also bridges plugin settings between the popup and the MAIN world.
 */
...
(function () {
  ...
  window.addEventListener('message', (event) => {
    ...
    if (event.data?.type === 'SUBMISSION_INTERCEPTED') {
      ...
      // Publish action beside CSSBattle's Submit button.
      chrome.runtime.sendMessage({
        type: 'SUBMISSION_CAPTURED',
        data: submissionData
      }).catch(err => console.error('[CSSBattle Archive] Failed to save draft:', err));
    }
  });
